...
void Square::setPieceAndColor(const Piece p, const Color c) {
    piece = p;
    color = c;
}
...
void Board::printBoard() const {
    ...
    for (int i = 0; i < 8; i++) {
        cout << " " << i << "   ";
        for (int j = 0; j < 8; j++) {
            const Piece p = square[i][j].getPiece();
            const Color c = square[i][j].getColor();
...
void Board::setBoard() {
    ...
    for (auto & i : square) {
        i[1].setPieceAndColor(PAWN, WHITE);
        i[6].setPieceAndColor(PAWN, BLACK);
    }

    for (int i = 2; i < 6; i++) {
        for (auto & j : square)
            j[i].setPieceAndColor(EMPTY, NONE);
    }
...
bool Board::moveQueen(Square* thisQueen, Square* thatSpace) const { //there might be problems with numbers of brackets
    //off board inputs should be handled elsewhere (before this)
    //squares with same color should be handled elsewhere (before this)

    const int queenX = thisQueen->getX();
    const int queenY = thisQueen->getY();
    const int thatX = thatSpace->getX();
    const int thatY = thatSpace->getY();
    std::cout << "this";
    ...
    if constexpr (constexpr bool invalid = false; invalid == false) {
...
bool Board::moveBishop(Square* thisBishop, Square* thatSpace) const { //there might be problems with number of brackets
    const int bishopX = thisBishop->getX();
    const int bishopY = thisBishop->getY();
    const int thatX = thatSpace->getX();
    if (const int thatY = thatSpace->getY(); abs(bishopX - thatX) == abs(bishopY - thatY)) {
        const int xIncrement = (thatX - bishopX) / (abs(thatX - bishopX));
        const int yIncrement = (thatY - bishopY) / (abs(thatY - bishopY));
...
bool Board::moveKnight(Square* thisKnight, Square* thatSpace)
{
    //off board inputs should be handled elsewhere (before this)
    //squares with same color should be handled elsewhere (before this)
    const int knightX = thisKnight->getX();
    const int knightY = thisKnight->getY();
    const int thatX = thatSpace->getX();

    if (const int thatY = thatSpace->getY(); (abs(knightX - thatX) == 2 && abs(knightY - thatY) == 1) || (abs(knightX - thatX) == 1 && abs(knightY - thatY) == 2)) {
...
bool Board::moveRook(Square* thisRook, Square* thatSpace) const {
    //off board inputs should be handled elsewhere (before this)
    //squares with same color should be handled elsewhere (before this)
    const int rookX = thisRook->getX();
    const int rookY = thisRook->getY();
    const int thatX = thatSpace->getX();
    if (const int thatY = thatSpace->getY(); rookX != thatX || rookY != thatY) {

        if (rookX == thatX) {
            const int yIncrement = (thatY - rookY) / (abs(thatY - rookY));
            ...
            const int xIncrement = (thatX - rookX) / (abs(thatX - rookX));
...
bool Board::movePawn(Square* thisPawn, Square* thatSpace) {
    ...
    //squares with same color should be handled elsewhere (before this)
    using namespace std;
    const int pawnX = thisPawn->getX();
    const int pawnY = thisPawn->getY();
    const int thatX = thatSpace->getX();
    const int thatY = thatSpace->getY();
...
bool Board::makeMove(const int x1, const int y1, const int x2, const int y2) {
